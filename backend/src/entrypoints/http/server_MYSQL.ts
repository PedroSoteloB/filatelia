...
const { db } = require('../../infrastructure/db/mysql/pool');
const bcrypt = require('bcrypt');
const jwt = require('jsonwebtoken');
const crypto = require('crypto');
const PptxGenJS = require('pptxgenjs'); // ⬅️ NUEVO: para generar PPT localmente
...
function toPublicUrl(p?: string | null): string | null {
  ...
  // ya viene como ruta pública
  if (s.startsWith('/uploads/')) return s;
  if (s.startsWith('uploads/')) return `/${s}`;

  // ruta absoluta dentro de la carpeta de uploads
...
// Reemplaza TODO el buildAttrJoins por esta versión "type-aware"
  ...
  for (const f of attrFilters) {
    ...
    if (Number.isFinite(Number(nameOrId))) {
      ...
      const [a]: any = await db.execute(
        `SELECT id FROM attribute_definitions WHERE owner_user_id = ? AND name = ? LIMIT 1`,
    ...
    if (op === 'between' && (f?.from != null) && (f?.to != null)) {
      ...
      if (typeof from === 'number' && typeof to === 'number' && Number.isFinite(from) && Number.isFinite(to)) {
        ...
        // entre textos no tiene sentido: fallback a texto exacto si coinciden
    ...
    // 3) Armar JOIN filtrando por attribute_id
    joins.push(`
      JOIN (
        SELECT ia.item_id
...
app.post('/_seed-admin', async (req: any, reply: any) => {
  try {
    ...
    const [roleRows]: any = await db.execute('SELECT id FROM roles WHERE name = ? LIMIT 1', ['admin']);
    if (roleRows.length) {
      await db.execute('INSERT IGNORE INTO user_roles (user_id, role_id) VALUES (?, ?)', [result.insertId, roleRows[0].id]);
    }
...
app.post('/auth/forgot-password', async (req: any, reply: any) => {
  try {
    ...
    const [rows]: any = await db.execute('SELECT id FROM users WHERE email = ? LIMIT 1', [email]);
    const user = rows[0];
    ...
    await db.execute(
      `INSERT INTO password_reset_tokens (id, user_id, token, expires_at, used) VALUES (UUID(), ?, ?, ?, 0)`,
...
app.post('/auth/reset-password', async (req: any, reply: any) => {
  try {
    ...
    const [rows]: any = await db.execute(
      `SELECT prt.user_id AS userId, prt.used, prt.expires_at AS expiresAt
       FROM password_reset_tokens prt WHERE prt.token = ? LIMIT 1`,
      [token]
    );
    const row = rows[0];
...
app.get('/roles', async (_req: any, reply: any) => {
  try { const [rows]: any = await db.execute('SELECT id, name, description FROM roles ORDER BY id ASC'); reply.send(rows); }
  catch (e:any) { reply.code(500).send({ message: e.message }); }
});
...
app.post('/roles', async (req: any, reply: any) => {
  try {
    ...
    const [r]: any = await db.execute('INSERT INTO roles (name, description) VALUES (?, ?)', [name, description || null]);
    reply.send({ id: r.insertId, name, description: description || null });
  } catch (e:any) { if (e.code === 'ER_DUP_ENTRY') return reply.code(409).send({ message: 'rol ya existe' }); reply.code(500).send({ message: e.message }); }
});
...
app.post('/roles/assign', async (req: any, reply: any) => {
  try {
    ...
    await db.execute('INSERT IGNORE INTO user_roles (user_id, role_id) VALUES (?, ?)', [userId, roleId]);
    reply.send({ ok: true });
  } catch (e:any) { reply.code(500).send({ message: e.message }); }
});
...
app.put('/roles/:id', async (req: any, reply: any) => {
  try {
    ...
    if (name) {
      const [dup]: any = await db.execute('SELECT id FROM roles WHERE name = ? AND id <> ?', [name, id]);
    ...
    const [rows]: any = await db.execute('SELECT id, name, description FROM roles WHERE id = ?', [id]);
...
app.delete('/roles/:id', async (req: any, reply: any) => {
  try { const id = Number(req.params.id); if (!id) return reply.code(400).send({ message: 'id inválido' }); await db.execute('DELETE FROM roles WHERE id = ?', [id]); reply.send({ ok: true }); }
  catch (e:any) { reply.code(500).send({ message: e.message }); }
});

app.get('/users', async (_req: any, reply: any) => {
  try { const [rows]: any = await db.execute('SELECT id, email, display_name FROM users ORDER BY id ASC'); reply.send(rows); }
  catch (e:any) { reply.code(500).send({ message: e.message }); }
});
...
app.post('/items', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    // Permitir crear sin imágenes si env/flag/query lo habilitan
    const allowNoImages =
    ...
    // Normaliza/impone visibilidad: siempre público
    const visibility = 'public';

    // Crear pieza principal (AHORA incluye visibility)
    ...
    if (Array.isArray(meta?.tags) && meta.tags.length) {
      ...
      for (const name of tagNames) {
        const [ex]: any = await db.execute(
          'SELECT id FROM tags WHERE owner_user_id = ? AND name = ? LIMIT 1',
          [ownerId, name]
        );
      ...
      for (const tid of Array.from(new Set(ids))) {
        await db.execute('INSERT IGNORE INTO item_tags (item_id, tag_id) VALUES (?, ?)', [itemId, tid]);
      }
    }
    ...
    if (Array.isArray(meta?.categories) && meta.categories.length) {
      for (const c of meta.categories) {
        ...
        await db.execute('DELETE FROM item_attributes WHERE item_id = ? AND attribute_id = ?', [itemId, attrId]);
        await db.execute(
...
app.get('/me/items', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const ownerId = ensureAuth(req);
    const sql = `
      SELECT i.id, i.title, i.country, i.issue_year AS issueYear,
             (SELECT file_path FROM item_images WHERE item_id=i.id ORDER BY is_primary DESC, id ASC LIMIT 1) AS cover
...
app.get('/items/search', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    if ((tagIds.length + tagNames.length) > 0) {
      ...
      if (uniqueIds.length) {
        if (tagMode === 'AND') {
          ...
          joinParams.push(...uniqueIds);      // 👈 van a joinParams
        } else {
          join += ` JOIN item_tags itf ON itf.item_id = i.id AND itf.tag_id IN (${uniqueIds.map(() => '?').join(',')})`;
    ...
    // 👇 orden correcto: primero joinParams, luego whereParams (ownerId, country, years, etc.)
...
//     const [rows]: any = await db.execute(
//       `SELECT i.*,
//               (SELECT JSON_ARRAYAGG(JSON_OBJECT('id',im.id,'file',im.file_path,'primary',im.is_primary))
//                FROM item_images im WHERE im.item_id = i.id) AS images
//        FROM philatelic_items i
//        WHERE i.id = ? AND i.owner_user_id = ?
//        LIMIT 1`,
//       [itemId, ownerId]
//     );
//     const item = rows?.[0]; if (!item) return reply.code(404).send({ message: 'not_found' });
//     reply.send(item);
//   } catch (e:any) { reply.code(500).send({ message: e?.message || 'internal_error' }); }
// });
// GET /items/:id  (mapea array de imágenes)
app.get('/items/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    const ownerId = ensureAuth(req);
    const itemId = Number(req.params.id);
    ...
    // MySQL puede devolver JSON ya parseado o string; normaliza:
    const imgs = typeof item.images === 'string' ? JSON.parse(item.images || '[]') : (item.images || []);
    item.images = imgs.map((im: any) => ({ ...im, file: toPublicUrl(im.file) }));
...
app.put('/items/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [r]: any = await db.execute(
      ...
      [title ?? null, description ?? null, country ?? null, issueYear ?? null, condition ?? null, catalogCode ?? null, faceValue ?? null, currency ?? null, acquisitionDate ?? null, id, ownerId]
    );
...
app.delete('/items/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [imgs]: any = await db.execute('SELECT id, file_path FROM item_images WHERE item_id = ?', [id]);
    ...
    const [r]: any = await db.execute('DELETE FROM philatelic_items WHERE id = ? AND owner_user_id = ?', [id, ownerId]);
...
app.post('/tags', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [dup]: any = await db.execute(
      `SELECT id FROM tags WHERE ${ownerFilter.where} AND name = ? LIMIT 1`,
...
app.get('/tags', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const ownerFilter = await tagsOwnerWhere(ownerId);
    const [rows]: any = await db.execute(
      `SELECT id, name FROM tags WHERE ${ownerFilter.where} ORDER BY name ASC`,
      ownerFilter.params
    );
    reply.send(rows);
  } catch (e:any) { reply.code(500).send({ message: e?.message || 'internal_error' }); }
...
app.delete('/tags/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    // (opcional: podríamos validar pertenencia si hay owner_user_id)
...
app.post('/items/:id/tags', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [it]: any = await db.execute('SELECT id FROM philatelic_items WHERE id = ? AND owner_user_id = ? LIMIT 1', [itemId, ownerId]);
    ...
    if (Array.isArray(tagNames) && tagNames.length) {
      ...
      if (names.length) {
        ...
        const [found]: any = await db.execute(
          `SELECT id, name FROM tags WHERE ${ownerFilter.where} AND name IN (${placeholders})`,
...
app.get('/attributes', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    const ownerId = ensureAuth(req);
    const [rows]: any = await db.execute(
      `SELECT id, name, attr_type AS attrType, options_json AS optionsJson, created_at AS createdAt
...
app.put('/attributes/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [ex]: any = await db.execute(
      `SELECT id FROM attribute_definitions WHERE id = ? AND owner_user_id = ? LIMIT 1`,
      [id, ownerId]
    );
    ...
    if (name) {
      const [dup]: any = await db.execute(
        `SELECT id FROM attribute_definitions WHERE owner_user_id = ? AND name = ? AND id <> ? LIMIT 1`,
        [ownerId, name, id]
      );
      if (dup.length) return reply.code(409).send({ message: 'attribute ya existe' });
    }
...
app.post('/items/:id/attributes', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    for (const a of attrs) {
      ...
      await db.execute('DELETE FROM item_attributes WHERE item_id = ? AND attribute_id = ?', [itemId, attributeId]);
      await db.execute(
        `INSERT INTO item_attributes (item_id, attribute_id, value_text, value_number, value_date)
         VALUES (?,?,?,?,?)`,
...
app.get('/items/:id/attributes', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [rows]: any = await db.execute(
      `SELECT ia.attribute_id AS attributeId, ad.name, ad.attr_type AS attrType,
              ia.value_text AS valueText, ia.value_number AS valueNumber, ia.value_date AS valueDate
       FROM item_attributes ia
...
app.delete('/items/:id/attributes/:attributeId', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    if (!Number.isFinite(itemId) || !Number.isFinite(attributeId)) return reply.code(400).send({ message: 'parámetros inválidos' });
...
app.post('/collections', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [r]: any = await db.execute(
      `INSERT INTO collections (owner_user_id, name, description, type, filter_json, sort_key, sort_dir)
       VALUES (?,?,?,?,?,?,?)`,
      [ownerId, name, description || null, type === 'static' ? 'static' : 'smart', filter_json ? JSON.stringify(filter_json) : null, sort_key, (String(sort_dir).toLowerCase() === 'desc' ? 'desc' : 'asc')]
    );
...
app.put('/collections/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    const ownerId = ensureAuth(req);
    const id = Number(req.params.id); if (!Number.isFinite(id)) return reply.code(400).send({ message: 'id inválido' });
    const { name, description, type, filter_json, sort_key, sort_dir } = req.body || {};

    const [c]: any = await db.execute('SELECT id FROM collections WHERE id = ? AND owner_user_id = ? LIMIT 1', [id, ownerId]);
...
//     const ownerId = ensureAuth(req);
//     const [rows]: any = await db.execute(
//       `SELECT id, name, description, type, filter_json, sort_key, sort_dir, created_at, updated_at
//        FROM collections WHERE owner_user_id = ? ORDER BY created_at DESC`,
//       [ownerId]
//     );
//     reply.send(rows);
//   } catch (e:any) { reply.code(500).send({ message: e?.message || 'internal_error' }); }
...
app.get('/collections', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [rows]: any = await db.execute(
      `SELECT
         id, name, description, type, filter_json, sort_key, sort_dir,
         created_at, updated_at, parent_collection_id, cover_image_path
       FROM collections
       WHERE owner_user_id = ?
...
app.get('/collections/:id/items', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [rows]: any = await db.execute(
      `SELECT id, type, filter_json, sort_key, sort_dir
         FROM collections
        WHERE id = ? AND owner_user_id = ?
        LIMIT 1`,
      [id, ownerId]
    ...
    if (col.type === 'static') {
      ...
      // ✅ DEFINIR f DESDE filter_json DELA COLECCIÓN
      let f: any = {};
      try {
...
app.post('/collections/:id/items', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    if (!Number.isFinite(id) || !Number.isFinite(Number(itemId))) return reply.code(400).send({ message: 'parámetros inválidos' });

    const [col]: any = await db.execute('SELECT id, type FROM collections WHERE id = ? AND owner_user_id = ? LIMIT 1', [id, ownerId]);
    ...
    await db.execute('INSERT IGNORE INTO collection_items (collection_id, item_id) VALUES (?, ?)', [id, itemId]);
    reply.send({ ok: true });
  } catch (e:any) { reply.code(500).send({ message: e?.message || 'internal_error' }); }
});
...
app.delete('/collections/:id/items/:itemId', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    if (!Number.isFinite(id) || !Number.isFinite(itemId)) return reply.code(400).send({ message: 'parámetros inválidos' });

    const [col]: any = await db.execute('SELECT id, type FROM collections WHERE id = ? AND owner_user_id = ? LIMIT 1', [id, ownerId]);
    if (!col.length) return reply.code(404).send({ message: 'collection_not_found' });
    if (col[0].type !== 'static') return reply.code(400).send({ message: 'solo para colecciones estáticas' });

    await db.execute('DELETE FROM collection_items WHERE collection_id = ? AND item_id = ?', [id, itemId]);
    reply.send({ ok: true });
  } catch (e:any) { reply.code(500).send({ message: e?.message || 'internal_error' }); }
});
...
app.delete('/collections/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [col]: any = await db.execute('SELECT id FROM collections WHERE id = ? AND owner_user_id = ? LIMIT 1', [id, ownerId]);
...
app.post('/saved-searches', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [r]: any = await db.execute(
      `INSERT INTO saved_searches (owner_user_id, name, filter_json) VALUES (?,?,?)`,
...
app.get('/saved-searches', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    const ownerId = ensureAuth(req);
    const [rows]: any = await db.execute(
      `SELECT id, name, filter_json, created_at FROM saved_searches WHERE owner_user_id = ? ORDER BY created_at DESC`,
      [ownerId]
    );
    reply.send(rows);
  } catch (e:any) { reply.code(500).send({ message: e?.message || 'internal_error' }); }
...
app.delete('/saved-searches/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [s]: any = await db.execute('SELECT id FROM saved_searches WHERE id = ? AND owner_user_id = ? LIMIT 1', [id, ownerId]);
...
app.get('/items/:id/tags', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [it]: any = await db.execute(
      'SELECT id FROM philatelic_items WHERE id = ? AND owner_user_id = ? LIMIT 1',
      [itemId, ownerId]
    );
    if (!it.length) return reply.code(404).send({ message: 'item_not_found' });
...
// SUB-BÚSQUEDA DENTRO DE UNA COLECCIÓN (sirve para SMART y SNAPSHOT)
app.get('/collections/:id/items/search-sub', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    if (col.type === 'static') {
      ...
      try {
        const raw = col.filter_json;
        fParent = raw == null ? {} : (typeof raw === 'string' ? JSON.parse(raw) : (Buffer.isBuffer(raw) ? JSON.parse(raw.toString('utf8')) : raw));
      } catch { fParent = {}; }
      ...
      if ((pTagIds.length + pTagNames.length) > 0) {
        ...
        if (unique.length) {
          if (String(pTagMode||'OR').toUpperCase()==='AND') {
            ...
            baseJoin += ` JOIN item_tags itf_parent ON itf_parent.item_id = i.id AND itf_parent.tag_id IN (${unique.map(()=>'?').join(',')})`;
            baseJoinParams.push(...unique);
          }
        }
      }
    ...
    if ((eTagIds.length + eTagNames.length) > 0) {
      ...
      if (unique.length) {
        if (String(eTagMode||'OR').toUpperCase()==='AND') {
          ...
          joinExtra += ` JOIN item_tags itf_extra ON itf_extra.item_id = i.id AND itf_extra.tag_id IN (${unique.map(()=>'?').join(',')})`;
          joinExtraParams.push(...unique);
        }
      }
    }
    ...
    const sql = `
      SELECT DISTINCT
        i.id, i.title, i.country, i.issue_year AS issueYear,
        (SELECT file_path FROM item_images WHERE item_id=i.id ORDER BY is_primary DESC, id ASC LIMIT 1) AS cover
      FROM philatelic_items i
      ${baseJoin}
      ${joinExtra}
...
app.post(
  ...
  async (req: any, reply: any) => {
    ...
    try {
      ...
      // ✅ Normalizamos la historia a string | null
      const historyText: string | null =
        history == null
          ? null
          : (String(history).trim() || null);

      const [crows]: any = await conn.execute(
        `SELECT id, type, filter_json, cover_image_path
           FROM collections
          WHERE id = ? AND owner_user_id = ?
          LIMIT 1`,
        [parentId, ownerId]
      ...
      // --- Portada base (si envían coverItemId, usar su primera imagen)
      ...
      if (String(mode).toLowerCase() === 'snapshot') {
        ...
        const [insCol]: any = await conn.execute(
          ...
          [
            ...
            name,
            description || null,
            historyText,            // ✅ se guarda la historia normalizada
            'static',
        ...
        await conn.execute(
          `INSERT IGNORE INTO collection_items (collection_id, item_id)
           VALUES ${values}`
        );
        ...
        // Obtener el id de la presentación y devolverlo
      ...
      // 4) Portada: si no se definió por coverItemId, hereda del padre
      if (!coverPath) {
        coverPath = parent.cover_image_path || null;
      }

      // 5) Insertar colección hija SMART
      const [insCol]: any = await conn.execute(
        `INSERT INTO collections
           (owner_user_id,
            name,
        ...
        [
          ...
          name,
          description || null,
          historyText,          // ✅ historia también para smart
          'smart',
      ...
      // 6) Crear presentación asociada si no existe
      await conn.execute(
        `INSERT INTO presentations (owner_user_id, collection_id, title, description, cover_image_path)
         SELECT ?, ?, ?, ?, ?
           FROM DUAL
      ...
      // 👀 log fuerte para ver el 500 real en consola
...
// Crea una presentación (opcional: subir portada vía multipart)
...
app.get('/presentations', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [rows]: any = await db.execute(
      `SELECT p.id, p.title, p.description, p.cover_image_path AS cover, 
              p.collection_id, p.created_at, p.updated_at
...
app.get('/presentations/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    const [row]: any = await db.execute(
      `SELECT p.id, p.title, p.description, p.cover_image_path AS cover, p.collection_id,
              p.created_at, p.updated_at,
              (SELECT COUNT(*) FROM presentation_assets a WHERE a.presentation_id = p.id) AS assetsCount
         FROM presentations p
        WHERE p.id = ? AND p.owner_user_id = ?
        LIMIT 1`,
      [id, ownerId]
    );
    if (!row.length) return reply.code(404).send({ message: 'not_found' });
    reply.send(row[0]);
  } catch (e:any) {
    reply.code(500).send({ message: e?.message || 'internal_error' });
  }
...
app.put('/presentations/:id', { preHandler: authGuard }, async (req: any, reply: any) => {
  try {
    ...
    let payload: any = {};
    let coverPath: string | undefined = undefined; // undefined=no cambio; null=limpiar
      ...
      for await (const p of parts) {
        ...
        if (p?.type === 'file' && p.fieldname === 'cover') {
          ...
          if (buf?.length) {
            const fs = require('fs'); const path = require('path');
            const base = process.env.FILES_BASE_PATH || path.join(process.cwd(), 'uploads');
            if (!fs.existsSync(base)) fs.mkdirSync(base, { recursive: true }); // ✅ fix mkdirSync
            const safe = `pres-cover-${Date.now()}-${(p.filename||'cover').replace(/[^\w.\-]+/g,'_')}`;
            const full = path.join(base, safe);
            fs.writeFileSync(full, buf);
            coverPath = full;
    ...
    const [ex]: any = await db.execute(
      'SELECT id, cover_image_path FROM presentations WHERE id = ? AND owner_user_id = ? LIMIT 1',
      [id, ownerId]
    );
    if (!ex.length) return reply.code(404).send({ message: 'not_found' });
  ...
  async (req: any, reply: any) => {
    try {
      ...
      const [p]: any = await db.execute(
        "SELECT id FROM presentations WHERE id = ? AND owner_user_id = ? LIMIT 1",
        [presId, ownerId]
      );
      if (!p.length) {
      ...
      const assets = rows.map((r: any) => ({
        ...
        filePath:  r.filePath,
        url:       r.url,
        metaJson:  parseMeta(r.metaJson),   // ⭐ aquí convertimos a objeto
      ...
      for await (const p of parts) {
        ...
        if (p?.type === 'file' && p.fieldname === 'file') {
          ...
          if (buf?.length) {
            ...
            // 🔹 AQUÍ EL CAMBIO: URL pública para que Angular tenga link
            url = `/uploads/${safe}`;
          }
          continue;
        }
...
////presentation
// --- helper para resolver rutas de imagen (absolutas o /uploads/xxx) ---
function resolveImagePath(p: any): string | null {
  if (!p) return null;
  const str = String(p);
  try {
    if (fs.existsSync(str)) return str; // ya es absoluta y existe
    const base = process.env.FILES_BASE_PATH || path.join(process.cwd(), 'uploads');

    // normaliza "/uploads/xxx.jpg" o "uploads/xxx.jpg"
...
// === Utils: aleatoriedad controlada y thumbs por tags ========================
...
// ⚠️ Arreglo: evitar T|undefined en swap con noUncheckedIndexAccess
const shuffleInPlace = <T,>(arr: T[], rand: () => number): T[] => {
  for (let i = arr.length - 1; i > 0; i--) {
    const j = Math.floor(rand() * (i + 1));
    // Non-null assertions: i y j siempre están en rango [0, length)
    const tmp = arr[i]!;
    arr[i] = arr[j]!;
    arr[j] = tmp;
  }
...
// Thumbs representativas por tags (si SMART). Fallback: covers de ítems.
async function representativeThumbs(
  ...
  const [rows]: any = await db.execute(
    `SELECT DISTINCT img.file_path AS url
       FROM item_tags it
...
function parseMeta(raw: any) {
  ...
  try {
    ...
    // si viene mal formado, devolvemos null para no romper el front
  }
  return null;
}
  ...
  async (req: any, reply: any) => {
    try {
      ...
      const [presRows]: any = await db.execute(
        `SELECT p.id, p.title, p.description, p.collection_id AS collectionId
           FROM presentations p
      ...
      // 2) Colección (ahora leemos history_text)
      ...
      const historyText: string = (col.history_text == null
        ? ""
        : String(col.history_text)
      ).trim();
      ...
      // 3) Ítems (igual que antes, solo para armar slides)
      let items: any[] = [];
      if (col.type === "static") {
        const [rows]: any = await db.execute(
          `SELECT i.id,
        ...
        if (tagIds.length + tagNames.length > 0) {
          ...
          if (unique.length) {
            if (String(tagMode || "OR").toUpperCase() === "AND") {
              ...
              join += ` JOIN item_tags itf ON itf.item_id = i.id AND itf.tag_id IN (${unique
                .map(() => "?")
                .join(",")})`;
              params.push(...unique);
            }
      ...
      // Helper: asegurar texto 25–300 caracteres
      ...
      // ✅ NUEVO helper: dividir historia por párrafos (punto aparte / saltos de línea)
      const splitHistoryByParagraphs = (text: string): string[] => {
        const raw = (text || "").replace(/\r\n/g, "\n").trim();
        if (!raw) return [];
        let parts = raw
          .split(/\n{2,}/) // doble salto de línea = nuevo párrafo
          .map((p) => p.trim())
          .filter(Boolean);
      ...
      // 4) Construir JSON para Presenton (slides)
      ...
      // 4.2 Historia (normalmente 1 slide; más solo si hay párrafos separados)
      const historyBlocks = splitHistoryByParagraphs(historyText);

      if (historyBlocks.length > 0) {
        // si toda la historia es cortita, la juntamos en un solo bloque
        const merged = historyBlocks.join("\n\n");
        const blocks =
          merged.length <= 400 ? [merged] : historyBlocks;

        // portadas de los ítems para ir rotando imagen
      ...
      // 4.3 Slides por ítem (detalle individual)
      ...
      // 4.4 Slide final: comparación entre las dos primeras piezas (si existen)
      if (items.length >= 2) {
        const a = items[0];
        const b = items[1];
...
`.trim();
      ...
      // 5) Llamar a Presenton
      const presentonRes = await createPresentationFromJson(body);

      // 6) Guardar asset como 'ppt' con URL externa
      const [ins]: any = await db.execute(
        `INSERT INTO presentation_assets (presentation_id, kind, file_path, url, meta_json)
         VALUES (?,?,?,?,?)`,
        [
...
app.get('/presentations/:id/ppt', { preHandler: authGuard }, async (req:any, reply:any) => {
  try {
    ...
    // 1) Verificamos que la presentación sea del usuario
    const [p]: any = await db.execute(
      'SELECT id FROM presentations WHERE id = ? AND owner_user_id = ? LIMIT 1',
      [presId, ownerId]
    );
    ...
    // 2) Buscamos el último asset tipo 'ppt' (el generado por Presenton)
    const [a]: any = await db.execute(
      `SELECT id,
              file_path AS filePath,
              url,
    ...
    // 3) Sacar edit_path de meta_json
    ...
    try {
      ...
      // si viene basura, simplemente no hay editPath
      editPath = null;
    }

    // 4) Construimos la URL de Presenton (Abrir / descargar en Presenton)
    ...
    // 5) Respondemos SOLO con URLs (nada de redirect ni stream)
    return reply.send({
      presentonUrl,           // 👉 para "Abrir / descargar en Presenton"
      downloadUrl: asset.url, // 👉 si algún día quieres usar el S3 directo
...
//       [presId, ownerId]
//     );
//     if (!p.length) return reply.code(404).send({ message: 'presentation_not_found' });

//     const [a]: any = await db.execute(
//       `SELECT id, file_path AS filePath, url
//          FROM presentation_assets
//         WHERE presentation_id = ? AND kind = 'ppt'
//         ORDER BY created_at DESC, id DESC
//         LIMIT 1`,
...
//     );
//     const asset = a?.[0];
//     if (!asset) return reply.code(404).send({ message: 'ppt_not_found' });

//     if (asset.url) {
//       return reply.redirect(302, asset.url);
//     }
//     if (asset.filePath) {
//       const fname = path.basename(asset.filePath);
...
//       return reply.send(fs.createReadStream(asset.filePath));
//     }

//     return reply.code(404).send({ message: 'ppt_not_found' });
//   } catch (e:any) {
//     req.log?.error(e, 'download-ppt failed');
//     return reply.code(500).send({ message: 'download_failed', detail: e?.message });
//   }
// });
