...
// 👇 IMPORTA SIEMPRE environment genérico
import { environment } from '../../../core/environments/environment.prod';

// Base del backend (se resuelve según el build: dev/prod)
const API_BASE = environment.apiBaseUrl;

// 👇 Log para verificar qué valor llega en producción
...
export class ForgotPasswordComponent {
  ...
  async submit() {
    ...
    try {
      ...
      // 👇 ahora apunta al backend según API_BASE
      const r = await fetch(`${API_BASE}/auth/forgot-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email })
      ...
      // En dev/prod: si el back devuelve resetLink, usarlo para navegar directo
      if (res.resetLink) {
        this.resetLink = res.resetLink;
        console.log('[DEV] resetLink:', this.resetLink);
