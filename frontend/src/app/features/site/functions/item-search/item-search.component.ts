...
//       // ✅ 0) Asegurar que results refleje el filtro actual
...
//       console.log('[snapshot] currentFilterJson (object):', filter);
  
//       const filterJson = JSON.stringify(filter);
//       console.log('[snapshot] filter_json (string):', filterJson);
...
//       console.log('[snapshot] payload to POST /collections:', payload);
...
//       console.log('[snapshot] /collections response:', created);
  
//       const collectionId = created?.id;
//       console.log('[snapshot] created collectionId:', collectionId);
...
//       console.log('[snapshot] items to link count:', items.length);
//       console.log('[snapshot] itemIds to link:', items.map(x => x.id));
  
//       for (const it of items) {
//         console.debug('[snapshot] linking item ->', { collectionId, itemId: it.id });
...
//             )
//           );
//           console.debug('[snapshot] linked OK itemId:', it.id, 'resp:', resp);
//         } catch (linkErr) {
//           console.error('[snapshot] FAILED linking itemId:', it.id, linkErr);
//           throw linkErr; // si prefieres que siga con los demás, quita este throw
//         }
//       }
  
//       console.log('[snapshot] all items linked. navigating to /collections');
//       console.groupEnd();
...
// ✅ AHORA soporta options (viene del options_json ya parseado desde el back)
type AttrDefDTO = {
  id: number;
  name: string;
...
export class ItemSearchComponent implements OnInit {
  ...
  // ✅ Draft de UI (para poder usar input o select en "Valor")
  attrDraftId: number | null = null;
  attrDraftOp: '=' | 'like' | 'between' = '=';
  attrDraftValue: string = '';
  attrDraftFrom: string = '';
  attrDraftTo: string = '';

  // ✅ atributo seleccionado (incluye options)
  selectedAttrDef = computed(() => {
    const raw = this.attrDraftId as any;
    const id = typeof raw === 'string' ? Number(raw) : raw;
    if (!id || !Number.isFinite(id)) return null;
    return (this.allAttrDefs() || []).find(d => d.id === id) ?? null;
  });

  // ✅ opciones disponibles para el campo "Valor"
  valueOptions = computed(() => {
    const opts = this.selectedAttrDef()?.options || [];
    return (opts || [])
      .map(x => String(x).trim())
      .filter(Boolean);
  });

  hasValueOptions = computed(() => this.valueOptions().length > 0);
  ...
  async loadAttrDefs() {
    try {
      ...
      // ✅ por si el back manda null o strings raros en options
  ...
  // 🔎 handler para el buscador de tags
  onTagSearchChange(evt: Event) {
    const v = (evt.target as HTMLInputElement)?.value ?? '';
    this.setTagSearch(v);
  }
  ...
  onAttrChange(evt: Event) {
    ...
    this.attrDraftId = Number.isFinite(id as any) ? (id as number) : null;

    // reset de inputs para que no queden valores viejos
    this.attrDraftValue = '';
    this.attrDraftFrom = '';
    this.attrDraftTo = '';
  }
  ...
  onAttrValueChange(evt: Event) {
    const v = (evt.target as HTMLInputElement)?.value ?? '';
    this.attrDraftValue = v;
  }

  onAttrFromChange(evt: Event) {
    const v = (evt.target as HTMLInputElement)?.value ?? '';
    this.attrDraftFrom = v;
  }
  ...
  async saveSearch(name: string) {
    ...
    try {
      ...
      const payload = { name: nm, filter_json: this.currentFilterJson() };
      await firstValueFrom(
        this.api.post('/saved-searches', payload, this.authHeaders())
      );
    } catch (e: any) {
      this.error.set(e?.message || 'No se pudo guardar la búsqueda');
    } finally {
